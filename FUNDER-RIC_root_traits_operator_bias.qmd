...
**Understanding the model structure.** The model `trait ~ siteID * treatment + operator` separates variance into three components:

- **`siteID * treatment`**: This captures all ecological variation including:
  - Main effect of each site (e.g., elevation, climate)
  - Main effect of each treatment (e.g., grazing, fertilization)
  - **Interaction between site and treatment** (treatment effects can differ by site)
- **`+ operator`**: An additive operator effect—a systematic measurement offset for each operator that applies across all sites and treatments. This assumes the operator bias is constant regardless of which site or treatment is measured.
- **Residual**: Unexplained variation (within-plot variability, random error)

**Key assumption:** The operator effect is additive (not interacted with site or treatment). This means we assume each operator has a consistent measurement bias across all conditions. This is a reasonable assumption for measurement bias and avoids overparameterization (a full `site × treatment × operator` interaction would require operators to measure all site × treatment combinations, which is not the case).
...
2. **Choose the reference operator.** The reference operator is the level to which we "correct" all values — we answer: "What would the value be if the reference operator had measured this plot?" **Important:** The reference operator should ideally have measured all (or most) site × treatment combinations so that their effect is estimated from the full range of ecological conditions. For this analysis, **Michaela** is the best choice as she has the most complete coverage across treatments. In the code, you can pass a different `reference_operator` to `apply_operator_correction()`.
...
**Does the correction preserve site × treatment variation?** The `siteID * treatment` term captures all ecological variation between sites and treatments, including their interaction. The correction only removes the operator offset component while keeping the site × treatment structure completely intact. Think of it as "removing measurement bias while preserving the ecological signal." The corrected values maintain all differences between sites and treatments; only the systematic measurement differences between operators are removed.
...
### Testing for treatment × operator interaction

The additive model assumes operator effects are constant across treatments. Here we test if operator effects vary by treatment, which would suggest treatment-specific measurement bias or confounding between operator and treatment.

```{r}
#| label: tbl-test-treatment-operator-interaction
if (run_operator_analysis && "dry_root_biomass_g" %in% names(dat)) {
  # Try to fit model with treatment × operator interaction
  d <- dat %>% filter(!is.na(dry_root_biomass_g))
  
  # Additive model (current approach)
  m_additive <- lm(dry_root_biomass_g ~ siteID * treatment + operator, data = d)
  
  # Model with treatment × operator interaction
  m_interaction <- tryCatch(
    lm(dry_root_biomass_g ~ siteID * treatment + treatment:operator, data = d),
  ...
  if (!is.null(m_interaction)) {
    # Compare models
    comparison <- tibble(
      Model = c("Additive (site × treatment + operator)", 
                "Interaction (site × treatment + treatment:operator)"),
      AIC = c(AIC(m_additive), AIC(m_interaction)),
      R_squared = c(summary(m_additive)$r.squared, summary(m_interaction)$r.squared)
    ) %>%
    ...
    knitr::kable(
      comparison,
      digits = 2,
      caption = "Comparing additive vs. treatment × operator interaction model. ΔAIC < -2 for interaction model suggests operator effects vary by treatment."
    )
  } else {
    knitr::kable(
      tibble(Message = "Interaction model failed to converge (likely due to sparse data in some treatment × operator combinations).")
    )
  }
}
```

**Interpretation:** ΔAIC for the interaction model is > -2, suggesting that operator effects do not vary by treatment.

### Alternative correction approach: subset-based comparison

To address concerns about confounding, we implement an alternative approach that estimates operator effects only from treatments where each operator overlaps with Michaela, avoiding extrapolation.

```{r}
#| label: alternative-correction
if (run_operator_analysis && "dry_root_biomass_g" %in% names(dat)) {
  # Alternative approach: subset-based correction
  # For each operator, calculate their effect relative to Michaela 
  # only in treatments where they both measured
  
  apply_subset_correction <- function(data, trait_name, reference_operator) {
    other_operators <- setdiff(levels(data$operator), reference_operator)
    
    # Start with reference operator's data (uncorrected)
    corrected_data <- data %>%
      filter(operator == reference_operator) %>%
      mutate(!!paste0(trait_name, "_subset_corrected") := .data[[trait_name]])
    
    # For each other operator
    operator_effects <- tibble(operator = character(), effect = numeric(), 
                               treatments_used = character(), n = integer())
    
    for (op in other_operators) {
      # Find treatments where both operator and reference measured
      op_treatments <- data %>% 
        filter(operator == op, !is.na(.data[[trait_name]])) %>% 
        pull(treatment) %>% unique()
      ref_treatments <- data %>% 
        filter(operator == reference_operator, !is.na(.data[[trait_name]])) %>% 
        pull(treatment) %>% unique()
      shared_treatments <- intersect(op_treatments, ref_treatments)
      
      if (length(shared_treatments) == 0) next
      
      # Subset to shared treatments
      subset_data <- data %>%
        filter(treatment %in% shared_treatments, 
               operator %in% c(op, reference_operator),
               !is.na(.data[[trait_name]]))
      
      if (nrow(subset_data) < 5) next
      
      # Fit model in subset
      subset_model <- lm(as.formula(paste(trait_name, "~ siteID * treatment + operator")), 
                        data = subset_data)
      
      # Get operator effect
      op_effect <- coef(subset_model)[paste0("operator", op)]
      
      # Store for comparison
      operator_effects <- bind_rows(
        operator_effects,
        tibble(operator = op, 
               effect = op_effect,
               treatments_used = paste(shared_treatments, collapse = ", "),
               n = nrow(subset_data %>% filter(operator == op)))
      )
      
      # Apply correction to this operator's data
      op_data <- data %>%
        filter(operator == op, !is.na(.data[[trait_name]])) %>%
        mutate(!!paste0(trait_name, "_subset_corrected") := .data[[trait_name]] - op_effect)
      
      corrected_data <- bind_rows(corrected_data, op_data)
    }
    
    list(corrected_data = corrected_data, operator_effects = operator_effects)
  }
  
  subset_result <- apply_subset_correction(dat, "dry_root_biomass_g", ref_operator)
  dat_subset_corrected <- subset_result$corrected_data
  
  # Compare operator effects from both methods
  # Calculate global method effects (same as the main operator effects table)
  d_global <- dat %>% filter(!is.na(dry_root_biomass_g))
  m_global <- lm(dry_root_biomass_g ~ siteID * treatment + operator, data = d_global)
  co_global <- coef(m_global)
  op_coefs_global <- co_global[grepl("^operator", names(co_global))]
  
  global_effects <- tibble(
    operator = gsub("operator", "", names(op_coefs_global), fixed = TRUE),
    global_effect = unname(op_coefs_global)
  )
  
  # Subset method effects
  subset_effects <- subset_result$operator_effects %>%
    rename(subset_effect = effect)
  
  # Merge for comparison
  effect_comparison <- full_join(global_effects, subset_effects, by = "operator") %>%
    mutate(difference = subset_effect - global_effect)
  
  knitr::kable(
    effect_comparison,
    digits = 3,
    caption = "Comparison of operator effects: Global model (all data) vs. Subset model (only shared treatments with Michaela). Large differences suggest confounding between operator and treatment."
  )
}
```
...
- **Global effect**: Operator effect estimated from all data (current approach)
- **Subset effect**: Operator effect estimated only from treatments where that operator and Michaela both measured
- **Difference**: If large (e.g., > 0.3g), suggests the operator effect varies by treatment or is confounded with treatment patterns
- **Treatments used**: Shows which treatments were used to estimate the subset effect

If differences are small, the additive assumption is reasonable. If differences are large, the subset approach is more conservative and avoids extrapolation.

### Operator effects (coefficients vs reference)

Estimated effect of each operator relative to the reference operator (`r ref_operator`) from the global model. Positive = higher trait value than reference on average (after accounting for site and treatment). The reference operator does not appear in the table because their effect is 0 by definition (they are the baseline).
