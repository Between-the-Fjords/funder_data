...
4. **Apply correction.** For each row measured by that operator: corrected = raw − (predicted with actual operator − predicted with reference operator), holding site and treatment fixed. For Michaela's rows: corrected = raw.
...
**Analysis subset.** 381 plots with RIC measurements and operator (Leo, Lou, Lucas, Michaela). Two plots in `root_traits_clean` have no RIC data and no operator (`Fau2GB`, `Ulv1B`); they are dropped.
...
if (run_operator_analysis && "dry_root_turnover_g" %in% names(dat)) {
  ...
  knitr::kable(
    ...
    digits = 3,
    caption = paste0(ref_operator, "'s measurement variability by treatment for RIC root turnover.")
  )
}
```
...
if (run_operator_analysis && "dry_root_turnover_g" %in% names(dat)) {
  ...
  knitr::kable(
    ...
    digits = 3,
    caption = paste0(ref_operator, "'s measurement variability by site for RIC root turnover. If CV is similar across sites, measurement consistency is good.")
  )
}
```
...
if (run_operator_analysis) {
  dat %>%
    filter(!is.na(dry_root_turnover_g)) %>%
    ggplot(aes(x = treatment, y = dry_root_turnover_g, fill = operator)) +
...
if (run_operator_analysis) {
  dat %>%
    ...
    ggplot(aes(x = treatment, y = avg_root_diameter_m, fill = operator)) +
    geom_boxplot(position = position_dodge(0.8), alpha = 0.8, outlier.alpha = 0.5) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
if (run_operator_analysis) {
  dat %>%
    filter(!is.na(dry_root_turnover_g)) %>%
    ggplot(aes(x = siteID, y = dry_root_turnover_g, fill = operator)) +
    geom_boxplot(position = position_dodge(0.8), alpha = 0.8, outlier.alpha = 0.5) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
if (run_operator_analysis && "dry_root_turnover_g_corrected" %in% names(dat_corrected)) {
  dat_corrected %>%
    filter(!is.na(dry_root_turnover_g)) %>%
    ggplot(aes(x = dry_root_turnover_g, y = dry_root_turnover_g_corrected, colour = operator)) +
...
if (run_operator_analysis) {
  ...
  ggplot(long_correction, aes(x = treatment, y = value, fill = operator)) +
    ...
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(x = "Treatment", y = "RIC root turnover (g)", fill = "Operator who measured", title = "Treatment vs RIC root turnover: uncorrected and corrected to Michaela")
}
```

## Effect of correction on treatment analysis

We fit the mixed model **RIC root turnover ~ treatment + (1|siteID)** once with raw values and once with operator-corrected values (corrected to **Michaela** as reference operator), to see how the correction affects the treatment effect and model statistics.
...
if (run_operator_analysis && exists("apply_operator_correction_local")) {
  ...
  dat_corrected_michaela <- apply_operator_correction_local(dat, "dry_root_turnover_g", reference_operator = ref_operator_analysis)
...
if (run_operator_analysis && exists("m_corrected")) {
  ...
  knitr::kable(fix_corr, digits = 4, caption = "Fixed effects: corrected RIC root turnover (reference: Michaela)", col.names = c("Term", "Estimate", "Std. Error", "df", "t value", "Pr(>|t|)"))
