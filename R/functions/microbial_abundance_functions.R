...
get_microbial_abundance_from_osf <- function(group, path) {

  # make path if required (thanks Richard)
  if(!dir_exists(path)){
    if(file_exists(path)){
  ...
  if(group == "bacteria") {
    # list only files in the directory of interest
    files <- osf_ls_files(node,
                          n_max = Inf,
                          path = "xii-xvi_soil_microbes_fungi/FUNDER_raw_microbial_abundance/bacteria/")
  }
  else if(group == "fungi") {
    # list only files in the directory of interest
    files <- osf_ls_files(node,
                          n_max = Inf,
                          path = "xii-xvi_soil_microbes_fungi/FUNDER_raw_microbial_abundance/fungi/")
  }
